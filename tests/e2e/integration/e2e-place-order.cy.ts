...
context('Order placement', () => {
  ...
  it('Should successfully place an order', function () {
    ...
    cy.get('#__nuxt .sf-header__icons .sf-button:nth-child(3)').click();
    // page.product.header.openCart();
    cy.get('#__nuxt a[href="/checkout"]').click();
    // page.cart.goToCheckoutButton.click();
    cy.get('#__nuxt #firstName').type("qwertry1");
    cy.get('#__nuxt #lastName').type("qwertry2");
    cy.get('#__nuxt #email').type("qwertry1@qwerty2.pl")
    cy.get('#__nuxt .form .sf-button').click();
